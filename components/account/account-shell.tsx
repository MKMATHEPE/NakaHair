  ...
  return (
    <main className="naka-account-layout">
      <aside className="naka-account-sidebar">
        ...
        <nav>{links.map(([href, label]) => <Link className={pathname === href ? "active" : ""} href={href} key={href}>{label}</Link>)}{user.isVendor ? <Link href="/vendor/products">Vendor Portal</Link> : <Link href="/account/vendor">Become a Vendor</Link>}<button className="naka-link-button" onClick={() => void logout()} type="button">Log out</button></nav>
      </aside>
