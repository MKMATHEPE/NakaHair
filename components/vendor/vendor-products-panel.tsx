  ...
  return (
    <>
      <div className="naka-panel-heading">
        ...
        <div className="naka-product-heading-actions">{savedMessage ? <span aria-live="polite">{savedMessage}</span> : null}<button className="naka-button-secondary" disabled={selectedTab === "catalogue"} onClick={() => openEditor(null)} title={selectedTab === "catalogue" ? "Choose a collection before adding a product" : undefined} type="button">{selectedTab === "catalogue" ? "Select a collection to add" : "+ Add Product"}</button></div>
      </div>
      ...
      {editor ? (
        <div className="naka-modal-backdrop" onMouseDown={closeEditor} role="presentation">
          <section aria-modal="true" className="naka-modal naka-editor-modal" onMouseDown={(event) => event.stopPropagation()} role="dialog">
            <button aria-label="Close editor" className="naka-modal-close" onClick={closeEditor} type="button">×</button>
            <p className="naka-eyebrow">Catalogue management</p>
            <h2>{editor.product ? `Edit ${editor.product.name}` : "Add Product"}</h2>
            ...
            <form className="naka-form naka-concept-editor" onInput={(event) => {
              ...
              <div className="naka-editor-workspace">
                <div className="naka-editor-fields">
                  ...
                  <section className="naka-editor-section">
                    <h3>2. Images</h3>
                    <label className="naka-image-upload">Product images (up to 8)<input accept="image/jpeg,image/png,image/webp,image/gif" multiple onChange={(event) => setPendingFiles(Array.from(event.target.files || []).slice(0, 8 - (editor.product?.image_urls?.length || 0)))} type="file" /><small>The first image is the main catalogue image. Each image must be 2 MB or smaller.</small></label>
                    {editor.product?.image_urls?.length || pendingFiles.length ? (
                      <div className="naka-image-manager">
                        {(editor.product?.image_urls || []).map((source, index) => <div key={source}><ProductImage alt={`${editor.product?.name} image ${index + 1}`} src={source} /><button aria-label={`Remove image ${index + 1}`} onClick={() => void removeImage(source)} type="button">×</button>{index === 0 ? <span>Main image</span> : null}</div>)}
                        {pendingFiles.map((file, index) => <div key={`${file.name}-${file.lastModified}`}><PendingImage file={file} /><button aria-label={`Remove ${file.name}`} onClick={() => setPendingFiles((files) => files.filter((entry) => entry !== file))} type="button">×</button>{!editor.product?.image_urls?.length && index === 0 ? <span>Main image</span> : null}</div>)}
                      </div>
                    ) : null}
                  </section>

                  <section className="naka-editor-section">
                    <h3>3. Options &amp; Pricing</h3>
                    <div className="naka-form-grid">
                      <label>Base price (ZAR) *<span className="naka-money-input"><b>R</b><input defaultValue={editor.product?.price} min={0} name="price" required step="0.01" type="number" /></span></label>
                    ...
                    {combinations.length ? <div className="naka-variant-table-wrap"><h4>Variant pricing</h4><table className="naka-variant-table"><thead><tr><th>Hair Origin</th><th>Size</th><th>Price</th></tr></thead><tbody>{combinations.map(({ origin, size }) => { const key = variantKey(origin, size); return <tr key={key}><td>{origin || "All origins"}</td><td>{size || "All sizes"}</td><td><span className="naka-money-input"><b>R</b><input aria-label={`${[origin, size].filter(Boolean).join(" ")} price`} defaultValue={editor.variants[key] ?? editor.product?.price ?? ""} min={0} name={`variant:${key}`} required step="0.01" type="number" /></span></td></tr>; })}</tbody></table></div> : <p className="naka-variant-hint">Select a hair origin or size to create variant pricing.</p>}
                  </section>

                  <section className="naka-editor-section">
                    <h3>4. Inventory &amp; Details</h3>
                    <div className="naka-form-grid">
                      <label>Stock quantity *<input defaultValue={editor.product?.stock_quantity || 0} min={0} name="stockQuantity" required type="number" /></label>
                      ...
                      <label className="naka-span-2">Colour<input defaultValue={editor.product?.details?.Colour || "Natural Black (1B)"} name="colour" /></label>
                    </div>
                  </section>
                </div>
                ...
                <aside className="naka-editor-preview">
                  ...
                  <div className="naka-editor-preview-card">
                    ...
                    <span>{preview.productType}</span>
                    <strong>{preview.name}</strong>
                    <p className="naka-editor-preview-price">{preview.oldPrice ? <del>{formatMoney(Number(preview.oldPrice))}</del> : null}{preview.price ? formatMoney(Number(preview.price)) : "R0"}</p>
                    <p>{preview.shortDescription}</p>
                    <dl><div><dt>Collection</dt><dd>{collectionLabel(editor.collection)}</dd></div><div><dt>Stock</dt><dd>{preview.stockQuantity}</dd></div></dl>
                  </div>
                </aside>
              </div>
