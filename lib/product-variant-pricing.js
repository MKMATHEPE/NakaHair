...
const calculateVariantPrice = (product, origin, size) => {
  ...
  const variantPrices = product?.variant_prices || product?.variantPrices;
  if (Array.isArray(variantPrices)) {
    const match = variantPrices.find((variant) =>
      String(variant?.hairOrigin || "") === String(origin || "")
      && String(variant?.size || "") === String(size || ""));
    if (match) return Math.round(asPrice(match.price, basePrice) * 100) / 100;
